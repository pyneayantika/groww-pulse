...
def generate_draft():
    try:
        ...
        if not themes:
            return jsonify({"error": "No themes found. Run pipeline first."}), 404

        theme_rows = ""
        for i, t in enumerate(themes[:3], 1):
            color = '#E24B4A' if t[1] >= 8 else '#F5A623' if t[1] >= 6 else '#00B386'
            theme_rows += f"""
            <tr>
                <td style="padding:8px;border-bottom:1px solid #eee">{i}. {t[0]}</td>
                <td style="padding:8px;border-bottom:1px solid #eee;color:{color}">{t[1]:.1f}/10</td>
                <td style="padding:8px;border-bottom:1px solid #eee">{t[4]}</td>
            </tr>"""

        actions = "".join(f"<li>{t[6]}</li>" for t in themes[:3] if t[6])

        quotes = "".join(f'''
                <blockquote style="border-left:3px solid #00B386;padding:8px 12px;
                    margin:8px 0;color:#555;font-style:italic;">"{t[5]}"</blockquote>'''
            for t in themes[:3] if t[5])

        body_html = f"""
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
            <div style="background:#00B386;padding:24px;border-radius:8px 8px 0 0">
                <h1 style="color:#fff;margin:0;font-size:20px">&#128202; Groww Weekly App Pulse</h1>
                <p style="color:rgba(255,255,255,0.85);margin:4px 0 0;font-size:13px">
                    Week {week_num}, {year} &middot; {total:,} reviews analyzed</p>
            </div>
            <div style="background:#fff;padding:24px;border:1px solid #eee">
                <h2 style="color:#00B386;font-size:15px">&#127919; Top 3 Themes This Week</h2>
                ...
                <h2 style="color:#00B386;font-size:15px">&#128172; What Users Are Saying</h2>
                {quotes}
            ...
            <div style="background:#f9f9f9;padding:14px;text-align:center;font-size:11px;
                ...
                Auto-generated by Groww Pulse Agent &middot; Every Monday 09:00 IST
            </div>
        </div>"""

        payload = {
            "subject": f"Groww App Pulse \u2014 Week {week_num}, {year} | Top Issue: {themes[0][0]}",
            "body_html": body_html,
            "body_text": f"Groww Weekly Pulse Week {week_num} - {total} reviews analyzed",
        }
